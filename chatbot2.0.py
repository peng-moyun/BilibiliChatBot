# -*- coding: utf-8 -*-
# 哔哩哔哩私信消息助手 V2.0.4
# 本程序可以实时获取B站私信新消息，并向您发送通知。
# 作者：两位B站用户：肥宅水水呀（https://space.bilibili.com/324042405）  编写代码主体
#                    wuziqian211（https://space.bilibili.com/425503913）       修改错误、完善代码

# 建议：以正则表达式方式标注“\\033\[1;3.*?m”、以普通方式标注“\033[0m”来增强可读性

import requests
import win32gui
import win32con
import win32com
import win32com.client
import os
import sys
import time
import base64
import win32clipboard
import win32api
import traceback  # 导入traceback module来提供一整套接口用于提取，格式化和打印Python程序的stack traces信息
import win32console
import colorama

try:
    appdata = os.getenv("Appdata")  # 获取环境变量Appdata的值
    TEMP = os.getenv("TEMP")  # 获取环境变量TEMP的值
    requests.packages.urllib3.disable_warnings()  # 禁用安全请求警告
    true = True
    false = False
    null = None
    colorama.init(autoreset=True)  # 初始化彩色文字，并且设置颜色自动恢复
    已输出的消息 = []


    # 彩色文字用法："\033[{显示方式};{前景色};{背景色}m" + 文字 + "\033[0m"
    # 数值表示的参数含义：
    # 显示方式  0（默认）   1（高亮）   22（非粗体）  4（下划线）  24（非下划线）  5（闪烁）   25（非闪烁）  7（反显）   27（非反显）
    # 前景色    30（黑色）  31（红色）  32（绿色）    33（黄色）   34（蓝色）      35（洋红）  36（青色）    37（白色）
    # 背景色    40（黑色）  41（红色）  42（绿色）    43（黄色）   44（蓝色）      45（洋红）  46（青色）    47（白色）

    class WindowsBalloonTip:
        def __init__(self):
            message_map = {win32con.WM_DESTROY: self.OnDestroy, }
            # 注册一个窗口类
            global wc
            wc = win32gui.WNDCLASS()
            self.hinst = wc.hInstance = win32gui.GetModuleHandle(None)
            wc.lpszClassName = "PythonTaskbar"
            wc.lpfnWndProc = message_map  # could also specify a wndproc.
            self.classAtom = win32gui.RegisterClass(wc)

        def ShowWindow(self, title, msg, survivaltime, infoflag):  # 标题，内容，消息存活时间，信息标志
            # 创建窗口
            style = win32con.WS_OVERLAPPED | win32con.WS_SYSMENU
            self.hwnd = win32gui.CreateWindow(self.classAtom, "Taskbar", style, \
                                              0, 0, win32con.CW_USEDEFAULT, win32con.CW_USEDEFAULT, \
                                              0, 0, self.hinst, None)
            win32gui.UpdateWindow(self.hwnd)

            # base64解码 ballontip.ico，存放于%TEMP%
            ballontip = base64.b64decode(
                "")
            with open(TEMP + "\\ballontip.ico", "wb") as f:
                f.write(ballontip)

            iconPathName = os.path.abspath(os.path.join(sys.path[0], TEMP + "\\ballontip.ico"))
            icon_flags = win32con.LR_LOADFROMFILE | win32con.LR_DEFAULTSIZE
            try:
                hicon = win32gui.LoadImage(self.hinst, iconPathName, \
                                           win32con.IMAGE_ICON, 0, 0, icon_flags)
            except:
                hicon = win32gui.LoadIcon(0, win32con.IDI_APPLICATION)
            flags = win32gui.NIF_ICON | win32gui.NIF_MESSAGE | win32gui.NIF_TIP
            nid = (self.hwnd,  # 句柄
                   0,  # 托盘图标ID
                   win32gui.NIF_INFO,  # 标识
                   win32con.WM_USER + 20,  # 回调消息ID
                   hicon,  # 托盘图标句柄
                   "Balloon tooltip",  # 图标字符串
                   msg,  # 气球提示字符串
                   200,  # 提示的显示时间
                   title,  # 提示标题
                   infoflag  # 提示用到的图标
                   )
            win32gui.Shell_NotifyIcon(win32gui.NIM_ADD,
                                      (self.hwnd, 0, flags, win32con.WM_USER + 20, hicon, "哔哩哔哩私信消息助手"))
            win32gui.Shell_NotifyIcon(win32gui.NIM_MODIFY, nid)

            if type(survivaltime) == int:
                time.sleep(survivaltime)
            else:
                time.sleep(5)  # 默认存活时间
            win32gui.DestroyWindow(self.hwnd)

        def OnDestroy(self, hwnd, msg, wparam, lparam):
            nid = (self.hwnd, 0)
            try:
                win32gui.Shell_NotifyIcon(win32gui.NIM_DELETE, nid)
                win32gui.PostQuitMessage(0)  # 终止消息
            except:
                pass


    w = WindowsBalloonTip()


    def title(text):  # 设置窗口标题
        try:
            win32console.SetConsoleTitle(text)
        except:
            pass


    def QQmsgsend(name, text):
        # 把文字放入剪贴板
        def setText(aString):
            win32clipboard.OpenClipboard()
            win32clipboard.EmptyClipboard()
            win32clipboard.SetClipboardData(win32con.CF_UNICODETEXT, aString)
            win32clipboard.CloseClipboard()

        # 模拟ctrl+V
        def ctrlV():
            win32api.keybd_event(17, 0, 0, 0)  # ctrl
            win32api.keybd_event(86, 0, 0, 0)  # V
            win32api.keybd_event(86, 0, win32con.KEYEVENTF_KEYUP, 0)  # 释放按键
            win32api.keybd_event(17, 0, win32con.KEYEVENTF_KEYUP, 0)

        # 模拟alt+s
        def altS():
            win32api.keybd_event(18, 0, 0, 0)
            win32api.keybd_event(83, 0, 0, 0)
            win32api.keybd_event(83, 0, win32con.KEYEVENTF_KEYUP, 0)
            win32api.keybd_event(18, 0, win32con.KEYEVENTF_KEYUP, 0)

        # 模拟alt+f4
        def altf4():
            win32api.keybd_event(18, 0, 0, 0)
            win32api.keybd_event(115, 0, 0, 0)
            win32api.keybd_event(115, 0, win32con.KEYEVENTF_KEYUP, 0)
            win32api.keybd_event(18, 0, win32con.KEYEVENTF_KEYUP, 0)

        # 模拟enter
        def enter():
            win32api.keybd_event(13, 0, 0, 0)
            win32api.keybd_event(13, 0, win32con.KEYEVENTF_KEYUP, 0)

        # 模拟单击
        def click():
            win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
            win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)

        # 移动鼠标的位置
        def movePos(x, y):
            win32api.SetCursorPos((x, y))

        # 获取QQ主界面窗口句柄
        hwnd = win32gui.FindWindow("TXGuiFoundation", "QQ")
        if hwnd == 0:
            print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31mQQ未运行，无法找到QQ主窗口句柄！\033[0m")
            return

        win32gui.ShowWindow(hwnd, win32con.SW_SHOW)

        # 设置QQ主界面为活动窗口
        win32gui.BringWindowToTop(hwnd)
        shell = win32com.client.Dispatch(
            "WScript.Shell")  # 先发送一个alt事件，否则会报错导致后面的设置无效：pywintypes.error: (0, 'SetForegroundWindow', 'No error message is available')
        shell.SendKeys('%')
        win32gui.SetForegroundWindow(hwnd)  # 设置为当前活动窗口

        time.sleep(1)  # 防止电脑较卡，没加载出窗口
        win32gui.MoveWindow(hwnd, 0, 0, 100, 100, True)
        time.sleep(0.01)
        # 1、移动鼠标至搜索框，单击，输入要搜索的名字
        movePos(100, 130)
        click()
        setText(name)
        ctrlV()
        time.sleep(1)  # 等待QQ进行搜索，其它位置的等待原因同样
        enter()  # 按enter键选定该联系人
        movePos(130, 285)  # 为防止QQ不跳出该联系人的聊天界面，使用鼠标双击打开
        click()
        click()
        time.sleep(1)
        # 2、点击屏幕中心，激活聊天窗口，复制要发送的消息，粘贴发送
        movePos(int(win32api.GetSystemMetrics(win32con.SM_CXSCREEN) / 2),
                int(win32api.GetSystemMetrics(win32con.SM_CYSCREEN) / 2))
        click()
        setText(text)
        altS()
        ctrlV()
        altS()
        time.sleep(2)
        # 先关闭聊天界面窗口
        altf4()
        # 再关闭QQ主窗口，以便下次运行
        try:
            win32gui.PostMessage(hwnd, win32con.WM_CLOSE, 0, 0)
        except:
            print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m关闭QQ主窗口时发生异常！\033[0m")


    def isConnected():  # 判断网络是否连接正常
        try:  # 异常调试
            requests.get("https://www.bilibili.com/", verify=False, proxies={'http': '', 'https': '', 'ftp': ''})
        except:  # 如果requests请求异常，执行except内的操作
            return False
        return True


    def fix():  # 异常修复
        print()
        print("\033[1;37m程序的配置损坏\033[0m，这通常是因为程序意外关闭导致的！")
        print("请重新填写数据（填写好的数据将会被记录，以方便下次使用）")
        print()
        if os.path.exists(appdata + "\\bilimsghelperdata.txt"):  # 如果配置存在
            os.remove(appdata + "\\bilimsghelperdata.txt")  # 删除配置文件

        settings()  # 跳转到设置


    def check():  # 检查配置文件是否存在，并检查Cookie是否失效
        if (os.path.exists(appdata + "\\bilimsghelperdata.txt")):  # 配置文件存在
            print("是否使用上次的配置？")

            while True:  # 一直循环
                用户选择 = input("输入1立即使用上次的配置，输入2则重新配置 [1/2] ")
                if 用户选择 == "1" or 用户选择 == "2":  # 用户的选择有效
                    break  # 跳出循环

                print("请输入数字：1或2！")  # 提示用户的选择无效，并再次循环

            if 用户选择 == "1":  # 使用上次的配置
                main()  # 跳转到主程序

            else:  # 重新配置
                if os.path.exists(appdata + "\\bilimsghelperdata.txt"):  # 如果配置存在
                    os.remove(appdata + "\\bilimsghelperdata.txt")  # 删除配置文件

                print()  # 输出空行
                print("请填写数据（填写好的数据将会被记录，以方便下次使用）")
                settings()  # 跳转到设置

        else:  # 配置文件不存在
            print("检测到第一次使用该程序，请先填写数据（填写好的数据将会被记录，以方便下次使用）")
            settings()


    def settings():  # 设置
        title("哔哩哔哩私信消息助手（状态：配置中）")
        print()
        print(
            "请在浏览器打开 B 站网页，按下 F12，点击“应用程序（Application）”，在左侧的“存储（Storage）”中点击“Cookie（s）”左边的箭头，然后在下面点击您访问的 B 站网页地址，在右侧表格中的“名称（Name）”一栏分别找到“SESSDATA”和“bili_jct”，分别 双击右边的“值（Value）”，然后复制这些值到本程序中。")
        SESSDATA = input("请输入SESSDATA的值：")
        bili_jct = input("请输入bili_jct的值：")

        Chanify_启用 = "否"
        Chanify_token = ""
        消息接收_启用 = "否"
        消息接收_别名 = ""
        消息接收_标签 = ""
        QQ_启用 = "否"
        QQ_name = ""

        print("是否将接收到的通知推送到您的手机？")
        while True:  # 一直循环
            用户选择 = input("输入1表示“是”，输入2表示“否” [1/2] ")
            if 用户选择 == "1" or 用户选择 == "2":  # 用户的选择有效
                break  # 跳出循环
            print("请输入数字：1或2！")  # 提示用户的选择无效，并再次循环

        if 用户选择 == "1":
            print('''\
选择一项：
    1、使用\033[1;37m“Chanify”\033[0m APP（仅支持苹果）：
    兼容性：
        iPhone
        设备需装有 iOS 14.0 或更高版本。
        iPad
        设备需装有 iPadOS 14.0 或更高版本。
        iPod touch
        设备需装有 iOS 14.0 或更高版本。


    2、使用\033[1;37m“消息接收”\033[0m APP（仅支持安卓，最好是小米）
    兼容性：Android版本 2.2+


    3、使用\033[1;37m“QQ”\033[0m（支持安卓+苹果）（\033[1;37m慎选\033[0m）
    说明：使用模拟点击方式发送消息，无需输入账号密码（可能会打断您电脑的正常使用，请谨慎选择！）
    要求：需确保电脑上已安装Windows最新版QQ，并登录
''')
            while True:  # 一直循环
                用户选择 = input(
                    "选择“Chanify”还是“消息接收”还是“QQ”？（1代表“Chanify”，2代表“消息接收”，3代表“QQ”）[1/2/3] ")
                if 用户选择 == "1" or 用户选择 == "2" or 用户选择 == "3":  # 用户的选择有效
                    break  # 跳出循环
                print("请输入数字：1或2或3！")  # 提示用户的选择无效，并再次循环

            if 用户选择 == "1":
                Chanify_启用 = "是"
                print('''
步骤1：打开APP Store，安装“Chanify”，完成后打开“Chanify”，允许通知，创建账号（点击即可创建，无需输入任何手机号的信息）
步骤2：打开Safari浏览器，输入“chanify://action/token/default”并回车\n步骤3：跳转到“Chanify”后会自动复制token，将其粘贴于下方
''')
                Chanify_token = input("请输入token：")
                while Chanify_token == "":
                    Chanify_token = input("您没有输入内容，请重新输入token：")

            if 用户选择 == "2":
                消息接收_启用 = "是"
                print('''
步骤1：在“酷安”APP 中搜索安装“消息接收”APP 或 访问“https://www.coolapk.com/apk/top.tdtt.news”下载安装
步骤2：打开“消息接收”APP，等待提示“注册小米推送服务成功”（可能需要些时间）
步骤3：点击“设置别名”和“订阅标签”按钮，自行设置内容（类似于设置接收代号，程序通过代号推送到您的设备），别名和标签尽量复杂以避免重复
步骤4：打开“消息接收”APP的通知权限
提醒：如您的手机不是小米手机，请不要清“消息接收”APP的后台！建议：关闭电池优化、后台下拉上锁
更多详细信息参照：https://blog.tdtt.top/article/6 中 “APP设置”
''')
                消息接收_别名 = input("请输入别名：")
                while 消息接收_别名 == "":
                    消息接收_别名 = input("您没有输入内容，请重新输入：")
                消息接收_标签 = input("请输入标签：")
                while 消息接收_标签 == "":
                    消息接收_标签 = input("您没有输入内容，请重新输入：")

            if 用户选择 == "3":
                QQ_启用 = "是"
                QQ_name = input(
                    "当程序接收到Bilibili新消息时，将消息发给QQ中的谁？（有备注则填备注，如没有填昵称，示例：我的Android手机）：")

        配置内容 = '''\
{
    "Push to phone":{
        "Chanify":{
            "启用":"''' + Chanify_启用 + '''",
            "token":"''' + Chanify_token + '''"
        },
        "消息接收":{
            "启用":"''' + 消息接收_启用 + '''",
            "别名":"''' + 消息接收_别名 + '''",
            "标签":"''' + 消息接收_标签 + '''"
        },
        "QQ":{
            "启用":"''' + QQ_启用 + '''",
            "name":"''' + QQ_name + '''"
        }
    },
    "Cookie":{
        "SESSDATA":"''' + SESSDATA + '''",
        "bili_jct":"''' + bili_jct + '''"
    }
}'''

        with open(appdata + "\\bilimsghelperdata.txt", "wt") as file:
            file.write(配置内容)
        print()
        print(
            '''[\033[1;37m提示\033[0m] \033[1;37m配置文件保存在 "''' + appdata + '''\\bilimsghelperdata.txt\033[0m"''')
        main()  # 跳转到主程序


    def main():  # 主程序
        with open(appdata + "\\bilimsghelperdata.txt", "rt") as x:
            配置内容 = x.read()

        try:  # 异常调试
            配置内容 = eval(配置内容)
            Chanify_启用 = 配置内容["Push to phone"]["Chanify"]["启用"]
            Chanify_token = 配置内容["Push to phone"]["Chanify"]["token"]
            消息接收_启用 = 配置内容["Push to phone"]["消息接收"]["启用"]
            消息接收_别名 = 配置内容["Push to phone"]["消息接收"]["别名"]
            消息接收_标签 = 配置内容["Push to phone"]["消息接收"]["标签"]
            QQ_启用 = 配置内容["Push to phone"]["QQ"]["启用"]
            QQ_name = 配置内容["Push to phone"]["QQ"]["name"]
            SESSDATA = 配置内容["Cookie"]["SESSDATA"]
            bili_jct = 配置内容["Cookie"]["bili_jct"]

        except:  # 如果读取配置文件异常，执行except内的操作
            fix()
            return

        headers = {"Cookie": "SESSDATA=" + SESSDATA + "; bili_jct=" + bili_jct,
                   "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.104 Safari/537.36"}

        # 获取API返回值（验证Cookie是否失效）
        用户登录数据 = eval(requests.get("https://api.bilibili.com/x/web-interface/nav", headers=headers, verify=False,
                                         proxies={'http': '', 'https': '', 'ftp': ''}).text)  # 获取当前用户登录信息
        UID = str(用户登录数据.get("data").get("mid"))  # 获取当前账号UID

        if 用户登录数据["code"] != 0:  # 账号未登录（-101），请求错误（-400），或者是其他原因导致的失败
            print("\033[1;37mcookie已失效或填写不正确！\033[0m")
            if os.path.exists(appdata + "\\bilimsghelperdata.txt"):  # 如果配置存在
                os.remove(appdata + "\\bilimsghelperdata.txt")  # 删除配置文件
            settings()  # 重新配置
            return

        if QQ_启用 == "是":
            print("\n\033[1;37m程序即将开始运行，请尽量最小化置顶窗口，以防止无法使QQ窗口置顶在最前端！\033[0m")

        时间戳_短 = eval(requests.get("http://api.bilibili.com/x/report/click/now", headers=headers, verify=False,
                                      proxies={'http': '', 'https': '', 'ftp': ''}).text).get("data").get("now")
        时间戳_长 = str(eval(
            requests.get("https://line1-log.biligame.net/collector/admin/config", headers=headers, verify=False,
                         proxies={'http': '', 'https': '', 'ftp': ''}).text).get("timestamp") * 1000)

        w.ShowWindow("开始运行", "开始运行！", 5, 1)
        print()

        while True:  # 无条件一直循环
            try:
                title("哔哩哔哩私信消息助手（状态：消息接收中）")
                消息列表数据 = eval(requests.get(
                    "https://api.vc.bilibili.com/session_svr/v1/session_svr/new_sessions?begin_ts=" + 时间戳_长 + "&build=0&mobi_app=web",
                    headers=headers, verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text)  # 获取消息列表

                if 消息列表数据.get("code") != 0:  # 如果“消息列表数据”的状态码不是“0”，报错
                    print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                        int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m" + 消息列表数据.get(
                        "message") + "\033[0m")
                    w.ShowWindow("程序发生异常，请检查程序运行状态！", "[错误] " + 消息列表数据.get("message"), 5, 2)
                    if Chanify_启用 == "是":
                        requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,
                                      data={'title': "程序发生异常，请检查程序运行状态！",
                                            'text': "[错误] " + 消息列表数据.get("message"), 'sound': 1}, verify=False,
                                      proxies={'http': '', 'https': '', 'ftp': ''}).text
                    if 消息接收_启用 == "是":
                        requests.get("https://tdtt.top/send", params={'alias': 消息接收_别名, 'topic': 消息接收_标签,
                                                                      'title': "程序发生异常，请检查程序运行状态！",
                                                                      'content': "[错误] " + 消息列表数据.get(
                                                                          "message")}, verify=False,
                                     proxies={'http': '', 'https': '', 'ftp': ''}).text
                    if QQ_启用 == "是":
                        QQmsgsend(QQ_name,
                                  ("程序发生异常，请检查程序运行状态！\n[错误] " + 消息列表数据.get("message")).encode(
                                      'gbk', 'replace').decode('gbk'))

                    if 消息列表数据.get("code") == -412:
                        print("\033[1;37m注：由于api访问频繁，程序将于15分钟后再试一次！\033[0m")
                        title("哔哩哔哩私信消息助手（状态：等待中）")
                        time.sleep(60 * 15)

                if 用户登录数据.get("code") != 0:  # 如果“用户登录数据”的状态码不是“0”，报错
                    print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                        int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m" + 用户登录数据.get(
                        "message") + "\033[0m")
                    w.ShowWindow("程序发生异常，请检查程序运行状态！", "[错误] " + 用户登录数据.get("message"), 5, 2)
                    if Chanify_启用 == "是":
                        requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,
                                      data={'title': "程序发生异常，请检查程序运行状态！",
                                            'text': "[错误] " + 用户登录数据.get("message"), 'sound': 1}, verify=False,
                                      proxies={'http': '', 'https': '', 'ftp': ''}).text
                    if 消息接收_启用 == "是":
                        requests.get("https://tdtt.top/send", params={'alias': 消息接收_别名, 'topic': 消息接收_标签,
                                                                      'title': "程序发生异常，请检查程序运行状态！",
                                                                      'content': "[错误] " + 用户登录数据.get(
                                                                          "message")}, verify=False,
                                     proxies={'http': '', 'https': '', 'ftp': ''}).text
                    if QQ_启用 == "是":
                        QQmsgsend(QQ_name,
                                  ("程序发生异常，请检查程序运行状态！\n[错误] " + 用户登录数据.get("message")).encode(
                                      'gbk', 'replace').decode('gbk'))

                    if 用户登录数据.get("code") == -412:
                        print("\033[1;37m注：由于api访问频繁，程序将于15分钟后再试一次！")
                        title("哔哩哔哩私信消息助手（状态：等待中）")
                        time.sleep(60 * 15)

                else:
                    if 消息列表数据.get("data").get("session_list") != None:  # 如果有新消息
                        for i in range(len(消息列表数据.get("data").get("session_list"))):
                            tallker_id = str(
                                消息列表数据.get("data").get("session_list")[i].get("talker_id"))  # 获取消息发送者UID
                            私信聊天数据 = eval(requests.get(
                                "https://api.vc.bilibili.com/svr_sync/v1/svr_sync/fetch_session_msgs?talker_id=" + tallker_id + "&session_type=1&size=20&build=0&mobi_app=web",
                                headers=headers, verify=False,
                                proxies={'http': '', 'https': '', 'ftp': ''}).text)  # 获取私信聊天记录
                            # 获取“消息列表数据”和“私信聊天数据”后，获取当前时间戳，作为临时暂存变量，传递给新一轮的“时间戳_短”和“时间戳_长”
                            临时时间戳_短 = eval(
                                requests.get("http://api.bilibili.com/x/report/click/now", headers=headers,
                                             verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text).get(
                                "data").get("now")
                            临时时间戳_长 = str(eval(
                                requests.get("https://line1-log.biligame.net/collector/admin/config", headers=headers,
                                             verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text).get(
                                "timestamp") * 1000)  # 获取当前时间戳，去除小数点

                            if 私信聊天数据.get("code") != 0:  # 如果状态码不是“0”，报错
                                print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                    int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m" + 私信聊天数据.get(
                                    "message") + "\033[0m")
                                w.ShowWindow("程序发生异常，请检查程序运行状态！",
                                             "[错误] " + 私信聊天数据.get("message"), 5, 2)
                                if Chanify_启用 == "是":
                                    requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,
                                                  data={'title': "程序发生异常，请检查程序运行状态！",
                                                        'text': "[错误] " + 私信聊天数据.get("message"), 'sound': 1},
                                                  verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text
                                if 消息接收_启用 == "是":
                                    requests.get("https://tdtt.top/send",
                                                 params={'alias': 消息接收_别名, 'topic': 消息接收_标签,
                                                         'title': "程序发生异常，请检查程序运行状态！",
                                                         'content': "[错误] " + 私信聊天数据.get("message")},
                                                 verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text
                                if QQ_启用 == "是":
                                    QQmsgsend(QQ_name, ("程序发生异常，请检查程序运行状态！\n[错误] " + 私信聊天数据.get(
                                        "message")).encode('gbk', 'replace').decode('gbk'))

                                if 私信聊天数据.get("code") == -412:
                                    print("\033[1;37m注：由于api访问频繁，程序将于15分钟后再试一次！\033[0m")
                                    title("哔哩哔哩私信消息助手（状态：等待中）")
                                    time.sleep(60 * 15)

                            else:
                                新消息数 = 0
                                while 私信聊天数据.get("data").get("messages")[新消息数].get("timestamp",
                                                                                             0) >= 时间戳_短:
                                    新消息数 += 1

                                for num in range(新消息数 - 1, -1, -1):
                                    sender_uid = str(私信聊天数据.get("data").get("messages")[num].get("sender_uid"))
                                    sender_info = eval(
                                        requests.get("https://api.bilibili.com/x/space/acc/info?mid=" + sender_uid,
                                                     headers=headers, verify=False,
                                                     proxies={'http': '', 'https': '', 'ftp': ''}).text)
                                    if sender_info.get("code") != 0:  # 如果“sender_info”的状态码不是“0”，报错
                                        print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                            int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m" + sender_info.get(
                                            "message") + "\033[0m")
                                        w.ShowWindow("程序发生异常，请检查程序运行状态！",
                                                     "[错误] " + sender_info.get("message"), 5, 2)
                                        if Chanify_启用 == "是":
                                            requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,
                                                          data={'title': "程序发生异常，请检查程序运行状态！",
                                                                'text': "[错误] " + sender_info.get("message"),
                                                                'sound': 1}, verify=False,
                                                          proxies={'http': '', 'https': '', 'ftp': ''}).text
                                        if 消息接收_启用 == "是":
                                            requests.get("https://tdtt.top/send",
                                                         params={'alias': 消息接收_别名, 'topic': 消息接收_标签,
                                                                 'title': "程序发生异常，请检查程序运行状态！",
                                                                 'content': "[错误] " + sender_info.get("message")},
                                                         verify=False,
                                                         proxies={'http': '', 'https': '', 'ftp': ''}).text
                                        if QQ_启用 == "是":
                                            QQmsgsend(QQ_name, (
                                                        "程序发生异常，请检查程序运行状态！\n[错误] " + sender_info.get(
                                                    "message")).encode('gbk', 'replace').decode('gbk'))

                                        if sender_info.get("code") == -412:
                                            print("\033[1;37m注：由于api访问频繁，程序将于15分钟后再试一次！\033[0m")
                                            title("哔哩哔哩私信消息助手（状态：等待中）")
                                            time.sleep(60 * 15)

                                    else:
                                        sender_name = str(sender_info.get("data").get("name"))

                                        if (sender_uid == UID) or (int(
                                                私信聊天数据.get("data").get("messages")[num].get("timestamp")) < round(
                                                float(时间戳_短))) or (
                                                私信聊天数据.get("data").get("messages")[num].get(
                                                        "msg_key") in 已输出的消息):  # 如果消息是当前账号发送的 或 消息时间戳小于本地时间戳 或msg_key存在于变量“已输出的消息”
                                            continue  # 跳过本次循环，直接进行下一轮循环
                                        else:
                                            if 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 1:  # 纯文本
                                                content = str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "content", "[未知]"))  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 2:  # 图片
                                                content = "[图片] " + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "url", "[未知]")).replace("\\/", "/")  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 5:  # 撤回消息
                                                content = "[撤回了一条消息]"
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 6:  # 自定义表情
                                                content = "[自定义表情] " + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "url", "[未知]"))  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 7:  # 分享稿件（除了直播）
                                                if eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("source") == 2:  # 相簿
                                                    content = "分享了相簿 id" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("id", "[未知]")) + "：" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("title", "[未知]"))  # 获取消息
                                                elif eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("source") == 5:  # 视频
                                                    content = "分享了视频 av" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("id", "[未知]")) + "：" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("title", "[未知]"))  # 获取消息
                                                elif eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("source") == 6:  # 专栏
                                                    content = "分享了专栏 cv" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("id", "[未知]")) + "：" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("title", "[未知]"))  # 获取消息
                                                elif eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("source") == 11:  # 动态
                                                    content = "分享了动态 id" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("id", "[未知]")) + "：" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("title", "[未知]"))  # 获取消息
                                                elif eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("source") == 16:  # 番剧/纪录片
                                                    content = "分享了" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("source_desc", "[未知]")) + " ep" + str(
                                                        eval(私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("id", "[未知]")) + "：" + str(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("title", "[未知]"))  # 获取消息
                                                else:
                                                    content = "分享了[未知]"
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 10:  # 通知消息
                                                details = ""
                                                for details_num in range(len(eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                                "content")).get("modules"))):
                                                    details = details + "                      " + eval(
                                                        私信聊天数据.get("data").get("messages")[num].get(
                                                            "content")).get("modules")[details_num].get("title") + "：" + \
                                                              eval(私信聊天数据.get("data").get("messages")[num].get(
                                                                  "content")).get("modules")[details_num].get(
                                                                  "detail") + "\n"
                                                content = "[通知消息] 标题：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "title", "[未知]")) + "\n           内容：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "text",
                                                    "[未知]")) + "\n           详细信息：\n" + details + "\n           " + str(
                                                    eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("jump_uri_config").get("text",
                                                                                               "[未知]")) + "：" + str(
                                                    eval(私信聊天数据.get("data").get("messages")[num].get(
                                                        "content")).get("jump_uri_config").get("all_uri",
                                                                                               "[未知]")) + "\n"
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 11:  # 发布视频
                                                content = "发布了视频 " + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "bvid", "[未知]")) + "：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "title", "[未知]"))  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 12:  # 发布专栏
                                                content = "发布了专栏 cv" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "rid", "[未知]")) + "：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "title", "[未知]"))  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 13:  # 卡片消息（abnormal-card）
                                                content = "[卡片消息] 标题：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "title", "[未知]")) + "\n           图片：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "pic_url", "[未知]")) + "\n           跳转链接：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "jump_url", "[未知]"))  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            elif 私信聊天数据.get("data").get("messages")[num].get(
                                                    "msg_type") == 14:  # 分享直播
                                                content = "分享了" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "author", "[未知]")) + "的直播：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "title", "[未知]")) + "（链接：" + str(eval(
                                                    私信聊天数据.get("data").get("messages")[num].get("content")).get(
                                                    "url", "[未知]")).replace("\\", "") + "）"  # 获取消息
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)


                                            else:
                                                content = "[不支持的消息类型]"
                                                已输出的消息.append(
                                                    私信聊天数据.get("data").get("messages")[num].get("msg_key"))
                                                消息时间戳 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                                                    私信聊天数据.get("data").get("messages")[num].get("timestamp")))
                                                w.ShowWindow(sender_name + "（UID" + sender_uid + "）", content, 5, 0)

                                            print((
                                                              "\033[1;37m[" + 消息时间戳 + "]\033[0m \033[1;36m" + sender_name + "\033[0m（\033[1;33mUID" + sender_uid + "\033[0m）：" + content).encode(
                                                'gbk', 'replace').decode('gbk'))  # 将无法解码的字符转换为“?”

                                            if Chanify_启用 == "是":
                                                requests.post("https://api.chanify.net/v1/sender/" + Chanify_token,
                                                              data={'title': sender_name + "（UID" + sender_uid + "）",
                                                                    'text': content, 'sound': 1}, verify=False,
                                                              proxies={'http': '', 'https': '', 'ftp': ''}).text
                                            if 消息接收_启用 == "是":
                                                requests.get("https://tdtt.top/send",
                                                             params={'alias': 消息接收_别名, 'topic': 消息接收_标签,
                                                                     'title': sender_name + "（UID" + sender_uid + "）",
                                                                     'content': content}, verify=False,
                                                             proxies={'http': '', 'https': '', 'ftp': ''}).text
                                            if QQ_启用 == "是":
                                                QQmsgsend(QQ_name,
                                                          (sender_name + "（UID" + sender_uid + "）：" + content).encode(
                                                              'gbk', 'replace').decode('gbk'))

                try:
                    时间戳_短 = 临时时间戳_短
                    时间戳_长 = 临时时间戳_长
                    del 临时时间戳_短, 临时时间戳_长  # 删除变量

                except:
                    时间戳_短 = eval(
                        requests.get("http://api.bilibili.com/x/report/click/now", headers=headers, verify=False,
                                     proxies={'http': '', 'https': '', 'ftp': ''}).text).get("data").get("now")
                    时间戳_长 = str(eval(
                        requests.get("https://line1-log.biligame.net/collector/admin/config", headers=headers,
                                     verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text).get(
                        "timestamp") * 1000)

                time.sleep(5)
            except:
                try:
                    时间戳_短 = 临时时间戳_短
                    时间戳_长 = 临时时间戳_长
                    del 临时时间戳_短, 临时时间戳_长  # 删除变量

                except:
                    try:
                        时间戳_短 = eval(
                            requests.get("http://api.bilibili.com/x/report/click/now", headers=headers, verify=False,
                                         proxies={'http': '', 'https': '', 'ftp': ''}).text).get("data").get("now")
                        时间戳_长 = str(eval(
                            requests.get("https://line1-log.biligame.net/collector/admin/config", headers=headers,
                                         verify=False, proxies={'http': '', 'https': '', 'ftp': ''}).text).get(
                            "timestamp") * 1000)
                    except:
                        pass

                time.sleep(5)


    # 程序开始运行时，运行以下代码
    title("哔哩哔哩私信消息助手")
    while True:  # 一直循环
        if isConnected() == true:  # 网络是否连接正常
            check()
        else:
            title("哔哩哔哩私信消息助手（状态：网络未连接）")
            input("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
                int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m无法连接到互联网，这会使任务无法完成！请检查网络是否正常！（按Enter键重试）\033[0m")

except KeyboardInterrupt:  # 用户按下“Ctrl+C”
    print()
    title("哔哩哔哩私信消息助手（状态：暂停运行）")
    print("\033[1;37m程序被用户终止运行！\033[0m")
    input("按下“Enter”键可重载程序！")
    print()
    os.execl(sys.executable, sys.executable, *sys.argv)

except:
    print()
    title("哔哩哔哩私信消息助手（状态：出现未知错误）")
    print("\033[1;37m[" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
        int(time.time()))) + "]\033[0m [\033[1;31m错误\033[0m] \033[1;31m程序运行时出现未知错误！\033[0m以下是错误信息：\n")
    traceback.print_exc(file=sys.stdout)  # 输出异常信息
    print()
    print('''\
您可以：
1、将以上错误信息通过B站私信到：肥宅水水呀（https://space.bilibili.com/324042405）或wuziqian211（https://space.bilibili.com/425503913）
或
2、在https://gitee.com/happycola/bili-msg-helper/issues中新建提交问题，并附上以上错误信息

来帮助我们完善这个程序鸭
''')
    input("按下“Enter”键可重载程序！")
    print()
    os.execl(sys.executable, sys.executable, *sys.argv)
